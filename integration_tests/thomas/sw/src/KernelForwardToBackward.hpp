#ifndef KERNEL_FORWARD_TO_BACKWARD_HPP
#define KERNEL_FORWARD_TO_BACKWARD_HPP

#include <string>
#include "experimental/xrt_kernel.h"
#include "experimental/xrt_uuid.h"

class KernelForwardToBackward {
    public:
        // Create XRT objects for device, kernel and run.
        KernelForwardToBackward(xrt::device device, xrt::uuid xclbin_uuid)
            : device(device) {
    ...
    private:
        ...
        inline static const std::string kernel_name = "forward_to_backward";

        // Indices of kernel arguments, must match the order in hw.
        static constexpr int arg_line_length         = 2;
        static constexpr int arg_num_lines_per_group = 3;
...
#endif // KERNEL_FORWARD_TO_BACKWARD_HPP