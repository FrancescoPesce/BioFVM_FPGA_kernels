#ifndef KERNEL_BACKWARD_PE_HPP
#define KERNEL_BACKWARD_PE_HPP

#include <string>
#include "experimental/xrt_kernel.h"
#include "experimental/xrt_uuid.h"

class KernelBackwardPE {
    public:
        // Create XRT objects for device, kernel and run.
        KernelBackwardPE(xrt::device device, xrt::uuid xclbin_uuid)
            : device(device) {
    ...
    private:
        ...
        inline static const std::string kernel_name = "backward_pe";

        // Indices of kernel arguments, must match the order in hw.
        static constexpr int arg_N = 4;
