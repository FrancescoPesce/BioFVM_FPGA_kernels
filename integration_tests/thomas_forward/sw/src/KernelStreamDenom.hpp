#ifndef KERNEL_STREAM_DENOM_HPP
#define KERNEL_STREAM_DENOM_HPP
...
class KernelStreamDenom {
    public:
        // Create XRT objects for device, kernel and run.
        KernelStreamDenom(xrt::device device, xrt::uuid xclbin_uuid)
            : device(device) {
        ...
        void set_inputs(ap_uint<REAL_WIDTH>* input, int line_length, int num_lines_per_S_group, int NS) {
            // Set banks for the input buffer.
            xrtMemoryGroup bank_input = krnl.group_id(arg_input);

            // Create the input buffer.
            buf_in = xrt::bo(device, NS * line_length * REAL_WIDTH / 8, xrt::bo::flags::normal, bank_input);
    ...
    private:
        ...
        inline static const std::string kernel_name = "stream_denom";
